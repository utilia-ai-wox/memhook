...
Each entry is `{dir, glob?, scope?, hostAutoLoaded?}` (defaults `*.md` / `memory`
/ `false`); `~` is expanded; only allowed-extension files are catalogued/injected
— `.md`, `.mdc`, or `.txt` (`SOURCE_EXTENSIONS`, the router `SAFE_BASENAME_RE`
floor, D36) — so a glob like `*.mdc` is now usable. They appear under a
`=== CUSTOM SOURCES ===` section and are
gated by `resurfaceHostLoaded` exactly like the rule zones. Resolution is a pure,
total transform in `src/sources.ts` (invalid entries dropped, never throws).

```yaml
customSources:
  - dir: ~/work/project/docs/decisions # any directory of .md notes
    glob: "*.md" # optional, default *.md
    scope: memory # optional: memory (default) | rules
    hostAutoLoaded: false # optional, default false
```
...
names: `cline`, `continue`, `copilot`, `windsurf` (project `.md`/`.instructions.md`
zones). Monolithic conventions (`AGENTS.md`, `GEMINI.md`, `~/.codex/AGENTS.md`,
…) are **not** presets — routing one big file whole is a no-op for relevance
selection; they are `/import` migration targets. Unknown names are dropped.
