...
a user can declare extra `.md` source directories in the YAML config so memhook
...
/ `false`); `~` is expanded; only `.md` files are catalogued/injected (router
`SAFE_BASENAME_RE`). They appear under a `=== CUSTOM SOURCES ===` section and are
...
filter shared with the catalog builder is `listMatchingMdFiles` (the `.md` gate +
glob), so detection and cataloguing can never disagree.
