...
while routing the manual/agent-requested rules in the same directory. Applied at
both catalog time (`emitCustomSourcesSection`) and router time (`readSelected`), so
the two stay in lockstep; `resurfaceHostLoaded` re-includes the always-applied
files exactly like the source-level gate. The predicate is pure + total (no/blank
frontmatter → false, the conservative direction). This is what makes the Cursor
preset faithful (its `.cursor/rules` mixes always-applied and manual rules).
...
cataloguing can never disagree.
