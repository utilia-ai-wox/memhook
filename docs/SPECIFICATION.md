...
**Custom sources (`customSources`, D31)**: beyond the built-in `~/.claude` zones,
a user can declare extra source directories in the YAML config so memhook
cables onto memory that already exists in a project (any directory, any naming).
Each entry is `{dir, glob?, scope?, hostAutoLoaded?, perFileAutoload?}` (defaults
`*.md` / `memory` / `false` / `false`); `~` is expanded; only allowed-extension
files are catalogued/injected — `.md`, `.mdc`, or `.txt` (`SOURCE_EXTENSIONS`, the
router `SAFE_BASENAME_RE` floor, D36) — so a glob like `*.mdc` is now usable. They
appear under a `=== CUSTOM SOURCES ===` section and are gated by
`resurfaceHostLoaded` exactly like the rule zones. Resolution is a pure, total
transform in `src/sources.ts` (invalid entries dropped, never throws).

**Per-file autoload (`perFileAutoload`, D37)**: where `hostAutoLoaded` skips a
whole directory, `perFileAutoload: true` decides skipping **per file** by
frontmatter — `isHostAutoloadedFile(content)` omits files the host already loads
in full every turn (Cursor `alwaysApply: true`, Windsurf `trigger: always_on`),
while routing the manual/agent-requested rules in the same directory. The skip is
decided **per directory** — a dir is skip-eligible when **any** active source over
it is `perFileAutoload` — and applied identically at catalog time
(`emitCustomSourcesSection`) and router time (`readSelected`) from the same
resolved source list, so the two never disagree even if two sources point at one
dir with different flags. `resurfaceHostLoaded` re-includes the always-applied
files exactly like the source-level gate. The predicate is pure + total (no/blank
frontmatter → false, the conservative direction; a trailing `# comment` after the
marker is tolerated). This is what makes the Cursor preset faithful (its
`.cursor/rules` mixes always-applied and manual rules).

```yaml
customSources:
  - dir: ~/work/project/docs/decisions # any directory of .md/.mdc/.txt notes
    glob: "*.md" # optional, default *.md
    scope: memory # optional: memory (default) | rules
    hostAutoLoaded: false # optional, default false — skip the whole dir
    perFileAutoload: false # optional, default false — skip always-applied files only
```

**Host presets (`presets`, D32)**: named bundles of `customSources` for a known
tool's atomic `.md` convention, so a user cables onto it without hand-writing
paths. Resolved by `expandPresets(names, cwd, home)` (`src/sources.ts`) at
catalog/router time (templates are `cwd`- or `home`-relative). Every preset is
**experimental** — doc-verified against the tool's official docs but not
live-tested (§24 "supported = tested"); a live echo-test promotes it. Built-in
names: `cline`, `continue`, `copilot`, `cursor`, `windsurf`. `cursor`
(`.cursor/rules/*.mdc`) and `windsurf` (`.windsurf/rules/*.md`) set
`perFileAutoload` (D37) so always-applied rules are skipped while manual ones
route; Cursor is top-level only (`globToRegExp` is basename-only — nested
`.cursor/rules/**` is unsupported) and plain `.md` there is ignored by Cursor, so
the glob is `*.mdc`. Monolithic conventions (`AGENTS.md`, `GEMINI.md`,
`~/.codex/AGENTS.md`, …) are **not** presets — routing one big file whole is a
no-op for relevance selection; they are `/import` migration targets. Unknown names
are dropped.

The special token **`auto`** (D35) is an explicit opt-in to zero-per-preset
config: instead of naming presets, the user lists `auto` and memhook routes every
preset it detects on disk (`detectPresets`, unioned with any explicit names).
`auto` is never the default — the cardinal opt-in (D31/D32) holds — and the routed
presets stay experimental (§24). Resolved by `resolveActivePresetNames(names, cwd,
home, readDir)`, which only touches disk when `auto` is present (a config without
`auto` pays zero detection I/O). `auto` + a `hostAutoLoaded` preset (e.g. `cline`)
still routes nothing under defaults — the same `resurfaceHostLoaded` gate applies.

```yaml
presets: [continue, cline] # enable built-in host presets (experimental)
# presets: [auto]          # or opt in once to every detected preset
```

**Preset discovery (`memhook presets`, D33)**: so a user need not know a preset's
name or hand-write its paths, `memhook presets list` shows every built-in preset
(name + summary + the dirs/globs it points at), and `memhook presets detect` scans
the project (cwd) + home for the preset directories that actually hold matching
`.md` files, then prints the `presets: [...]` snippet to enable the ones found.
Detection is a pure transform `detectPresets(cwd, home, readDir)` (`src/sources.ts`),
the real `readdirSync` injected as a seam; it is total (a missing/denied dir is
recorded as absent, never throws). `presets` is not on the hook path (§9). The
filter shared with the catalog builder is `listMatchingFiles` (the
`SOURCE_EXTENSIONS` floor — `.md`/`.mdc`/`.txt` — + glob, D36), so detection and
cataloguing agree on which files _match_. Detection is **content-blind** by design
(glob only, no file read): for a `perFileAutoload` preset it reports a dir's
presence, not its routable count, so a `.cursor/rules` holding **only**
always-applied rules is reported (and may be nudged) even though enabling it routes
nothing once the per-file skip runs. Making detection content-aware would add a
file read per candidate to a readdir-only path (and to the per-prompt nudge), so it
is deliberately left as a presence signal.

**Presets nudge (`maybePresetsNudge`, D34)**: so the discovery command does not
itself need to be known, the router (on the hook path) attaches an additive
one-line `systemMessage` when a known host's memory directory exists in the
project (or home) but no `presets:` entry routes it yet — suggesting `memhook
presets detect`. It **only suggests** the explicit `presets:` config; it never
auto-routes (every preset is experimental, §24) and it skips a host already routed
by a named preset **or** by an equivalent hand-written `customSources` dir. Same
mechanism and guard-rails as the `/curate` nudge ([§26](#26-companion-skills-v04)):
local-only (no outbound call), additive (§10.2), fully wrapped so it never affects
fail-soft (§6.1). The cooldown stamp is keyed by cwd (the signal is per-project,
unlike the catalog-size one) and, like the `/curate` nudge, is written only on a
fire — so it throttles re-firing within the window rather than gating the
detection itself. `systemMessage` is a single channel, so the presets nudge is a
fallback — it fires only on a turn the `/curate` nudge did not. Toggle
...
| `MEMHOOK_PRESETS_NUDGE`               | `true`                                    | bool   | Enable the presets nudge (§8.2, D34): suggest `memhook presets detect` when a known host's memory exists but isn't routed. Local-only, additive, opt-in. |
| `MEMHOOK_PRESETS_NUDGE_COOLDOWN_DAYS` | `7`                                       | int    | Minimum days between presets nudges.                                                                                                                     |
...
- File basenames validated against `^[A-Za-z0-9._-]+\.(md|mdc|txt)$`
  (`SAFE_BASENAME_RE`, built from `SOURCE_EXTENSIONS`, D36) — no slashes,
  no traversal. Widening the extension set (D36) preserves the
  no-slash/no-traversal property; only the trailing extension changed.
...
> source extensions (D36) + a per-file autoload predicate, a Cursor `.mdc` preset,
> and Windsurf refined to per-file (D37). **npm publish is live and automated** (Trusted
...
| **v0.6.0**           | Widened `.md`/`.mdc`/`.txt` source extensions (D36); per-file autoload predicate + a Cursor `.mdc` preset, Windsurf refined to per-file (D37). All on `main`.                                                                                                                                                                                   | in progress     |
...
| D31 | 2026-06-02 | Custom memory sources (`customSources`, YAML-only)                                                     | memhook installs mid-project, so memory already exists. `customSources` lets a user point memhook at extra `.md` directories (any naming, via a glob) beyond the built-in `~/.claude` zones, so it cables onto existing project memory. Each `{dir, glob?, scope?, hostAutoLoaded?}` is resolved by a pure/total transform (`src/sources.ts`); the catalog adds a `CUSTOM SOURCES` section and the router scans the dirs, both gated by `resurfaceHostLoaded`. YAML-only (lists don't map cleanly to env vars), default empty — additive, no behaviour change without config. First step of the generalised source registry (the per-host presets + `memhook import` follow).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| D32 | 2026-06-02 | Built-in host presets (`presets`, YAML-only, all experimental)                                         | Named bundles of `customSources` for known tools' atomic `.md` conventions (`cline`, `continue`, `copilot`, `windsurf`), so users cable on by name instead of hand-writing paths. Sources doc-verified against each tool's official docs (workflow `wf_6e20f9d2`, persisted in `docs/private`), but NOT live-tested → every preset ships **experimental** (§24); a live echo-test promotes it. Expanded by a pure transform `expandPresets(names, cwd, home)` (`src/sources.ts`) at catalog/router time. Monoliths (`AGENTS.md`/`GEMINI.md`/Codex) are excluded — they are `/import` migration targets, not sources. Cursor (`.mdc`) is deferred pending a non-`.md` extension change (PRE-A) + per-file autoload predicate (PRE-B). YAML-only, default empty, additive.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| D33 | 2026-06-03 | Preset discovery command (`memhook presets list\|detect`)                                              | Makes D31/D32 zero-friction: instead of knowing a preset's name and hand-writing `presets: [continue]`, the user runs `memhook presets detect`, which scans the project + home for the preset directories that hold matching `.md` and prints the `presets: [...]` snippet to enable them (`list` documents all built-ins). Detection is a pure transform `detectPresets(cwd, home, readDir)` (`src/sources.ts`), `readdirSync` injected as a seam, total (a missing dir → absent, never throws); `src/presetsCmd.ts` is the I/O shell. The catalog's `.md`+glob filter is extracted to the shared pure `listMatchingMdFiles`, so detection and cataloguing can never disagree. `presets` is NOT on the hook path → may exit non-zero (§9 boundary). Additive, no behaviour change to `run`/`build-catalog`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| D34 | 2026-06-03 | Presets nudge (`maybePresetsNudge`, additive `systemMessage`)                                          | Makes the D33 discovery command self-announcing: on the hook path the router attaches a one-line `systemMessage` when a known host's memory directory exists in the project/home but no `presets:` entry routes it yet, suggesting `memhook presets detect`. It only **suggests** the explicit `presets:` config — it never auto-routes (every preset is experimental, §24), so the opt-in design (D31/D32) is preserved. Same shape and guard-rails as the `/curate` nudge (D26): local-only (§6.2), additive (§10.2), fully wrapped (§6.1). The cooldown stamp is keyed by cwd (a per-project signal) and written only on a fire, so it throttles re-firing, not the detection; the nudge also skips a host already routed by a named preset or an equivalent `customSources` dir. As `systemMessage` is one channel, it is a fallback to the `/curate` nudge (fires only when that did not). Toggle `MEMHOOK_PRESETS_NUDGE`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| D35 | 2026-06-03 | `presets: [auto]` token — opt-in zero-per-preset routing                                               | Caps the preset chain (D32 name → D33 detect → D34 nudge) with one-token enablement: `presets: [auto]` routes every preset detected on disk instead of naming each. Chosen over auto-routing-by-default, which was rejected as it would violate the cardinal opt-in (D31/D32 — _no behaviour change without config_) and auto-activate experimental, non-live-tested path matching (§24). `auto` is an **explicit** opt-in, never the default, so both hold; routed presets stay experimental. Resolved by the pure `resolveActivePresetNames(names, cwd, home, readDir)` (`src/sources.ts`), which `detectPresets` only when `auto` is present (zero detection I/O otherwise) and unions with explicit names, de-duped. `resolveSources` now takes the `readDir` seam so catalog + router expand `auto` identically (the invariant). `hostAutoLoaded` presets still gate on `resurfaceHostLoaded` (no double-injection, D30). YAML-only, default empty, additive. Challenge: `docs/private/A6b-autoload-CHALLENGE-2026-06-03.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| D36 | 2026-06-03 | Widen routable source extensions to `.md` / `.mdc` / `.txt` (`SOURCE_EXTENSIONS`)                      | The catalog file filter and the router injection guard were both hardwired to `.md`, blocking other tools' atomic rule files (Cursor `.mdc`, Cline `.txt`). Introduce one source of truth `SOURCE_EXTENSIONS = [md, mdc, txt]` (`src/sources.ts`); the shared filter (renamed `listMatchingMdFiles` → `listMatchingFiles`) and the router `SAFE_BASENAME_RE` are both built from it, so the catalog can never list a file the router would refuse (no drift). The extension set is a safety floor independent of the glob (a `*` glob still yields only these); the no-slash/no-traversal security property (§19.2) is unchanged — only the trailing extension widened. Claude Code's own rule zones + memhook's `feedback_*/project_*` memory stay `.md`-only. Foundation (PRE-A) for a future Cursor `.mdc` preset (which additionally needs a per-file autoload predicate, PRE-B); immediately usable now via a `customSources` `*.mdc`/`*.txt` glob. Additive — existing `.md` presets/sources unchanged (their glob still narrows to `.md`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| D37 | 2026-06-04 | Per-file autoload predicate (`perFileAutoload`, PRE-B) + Cursor preset; Windsurf refined to per-file   | `hostAutoLoaded` skips a whole source directory, but Cursor's `.cursor/rules` mixes always-applied rules (`alwaysApply: true` — host loads them every turn) with manual/agent-requested ones (host never surfaces them). A faithful Cursor preset needs a **per-file** skip. Add `perFileAutoload` to `CustomSource`/`PresetSourceDef` and a pure/total predicate `isHostAutoloadedFile(content)` (`src/sources.ts`) that reads only the YAML frontmatter for the two documented per-file always-on markers — Cursor `alwaysApply: true`, Windsurf `trigger: always_on` (host-source-presets-SPEC footnotes 5–6). It is applied **per directory** (a dir is skip-eligible iff any active source over it is `perFileAutoload`) identically at catalog time (`emitCustomSourcesSection`) and router time (`readSelected`), both from the same resolved source list, so the two never disagree even when two sources point at one dir with different flags; gated by `!resurfaceHostLoaded` (which re-includes the always-applied files, mirroring the source-level gate). The frontmatter-block boundary is a shared `frontmatterBlock(content)` helper used by both the predicate and the catalog's description extractor, so they can't desync. The predicate tolerates a trailing `# comment` after the marker. Note: `detectPresets` / the presets nudge stay content-blind (glob only) — they report a preset dir's presence, not its routable count, so a dir of only always-applied rules is reported but routes nothing (adding a file read to the detect/nudge path was judged not worth it). New `cursor` preset (`.cursor/rules/*.mdc`, `perFileAutoload: true`, top-level only — `globToRegExp` is basename-only so nested `**` is unsupported; plain `.md` there is ignored by Cursor, hence `*.mdc`). The existing `windsurf` preset is refined to `perFileAutoload: true` (its `always_on` files are likewise host-loaded). Both stay **experimental** — doc-verified, no live echo-test (§24). Pure predicate fails conservative (no/blank frontmatter → route, never wrongly hide). Additive, YAML-only field, default false. |
